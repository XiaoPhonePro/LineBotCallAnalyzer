...
def summarize_text(text: str) -> str:
    ...
    2.  重點摘要 (繁體中文)：
        * 接下來，請把這段文字的主要內容翻譯成**繁體中文**，並且整理出一個簡潔的重點大綱或摘要。
        * 目標是讓我能快速抓住核心資訊，所以請幫我把重點條理分明地列出來，或者用一段流暢的話總結。
        * 摘要的長度請盡量控制在 100 字左右就好，如果原文比較短，摘要就簡短一點也沒關係。
        * 為了讓內容看起來更活潑，可以在摘要中穿插 1 到 2 個相關的表情符號 (emoji) 喔！😉
        * **輸出格式**：請確保最終的摘要是**純文字 (plain text)**，不要包含任何 Markdown 格式的標記 (例如 `*`、`#`、`-`、`[]()` 等)，讓輸出看起來乾淨整潔。

    3.  結語：完成以上步驟後，請用親切友善的語氣呈現結果，並在最後加上一句俏皮的結語，暗示任務已經快速有效率地完成了，例如：「咻！我已經幫您把重點整理好囉！希望對您有幫助！🚀」或者「搞定！以上是為您整理的重點，希望能節省您的時間！✨」。
